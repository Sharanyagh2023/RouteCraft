...
export class ScoringEngine {
  ...
  static getCheapest(routes: RouteOption[]): RouteOption[] {
    ...
    // Find the cheapest multi-modal (split) route
    const firstSplitIdx = sorted.findIndex(r => r.type === "split");
    
    if (firstSplitIdx > 0) {
      // Hoist the cheapest multi-modal route to the top as requested
      const cheapestSplit = sorted.splice(firstSplitIdx, 1)[0];
      return [cheapestSplit, ...sorted];
    }
